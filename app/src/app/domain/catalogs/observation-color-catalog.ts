...
export function findLayoutVariantsForObservation(
  mappings: readonly ObservationColorMapping[],
): readonly SideColorLayout[] {
  const seen: SideColorLayout[] = [];

  for (const mapping of mappings) {
    if (!seen.some((layout) => layoutsAreEqual(layout, mapping.layout))) {
      seen.push(mapping.layout);
    }
  }
